...
print(Float(humanOfficers.filter({femaleOfficers.contains($0)}).count) / Float(humanOfficers.count) * 100)

// Q2: How many first officers are human?

print(firstOfficers.keys.filter({femaleOfficers.contains($0)}).count)
